...
Above, I signed my promise digitally by PGP almost the same way as I do a traditional handwritten signature. In order to sign I used [Keybase](https://keybase.io/sign){:target="_blank"}, it can produce a PGP signature.
...
* `hashAlgorithm` - the name of algorithm that was used to hash text before signature, in my case `SHA512`. Wait a minute, I already have seen it just after `-----BEGIN PGP SIGNED MESSAGE-----`. The answer is ["Hash:" header can be considered as a remnant of older times](https://security.stackexchange.com/a/43333/207381){:target="_blank"}.
* `creationTime` - the date when signature has been created. The field is also signed so it is [trustworthy](https://crypto.stackexchange.com/a/2676){:target="_blank"} (if you trust the signer).
* `keyId` - ID of the key that signed the text, in my case `f3e48ae115e03e71`. In order to verify the signature, the public key can be downloaded [from Keybase](https://keybase.io/_/api/1.0/key/fetch.json?ops=4&pgp_key_ids=f3e48ae115e03e71){:target="_blank"} via [Keybase API](https://keybase.io/docs/api/1.0/call/key/fetch){:target="_blank"}. Moreover, `Keybase` provides complete information about [the owner of the key](https://keybase.io/_/api/1.0/user/lookup.json?usernames=gaevoy){:target="_blank"} via [Keybase API](https://keybase.io/docs/api/1.0/call/user/lookup){:target="_blank"}, so anyone can prove that the signer is legitimate, looking to the connected entities. 
...
It is interesting what parts can be removed. I don't expect to be compatible with PGP standard after that. However, the reduced version should be convertible back to the original. One more open source tool will help me to prototype. Greet, [CyberChef](https://gchq.github.io/CyberChef/){:target="_blank"}!
